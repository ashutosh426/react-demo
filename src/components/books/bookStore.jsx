...
class BookStoreClass extends Component {
    state = { 
     ...
     handleDelete=(eachBook)=>{
        //here we should remove the book from the array
        console.log("parent component delete  is called");
    ...
    renderAllBooks(){
        return this.state.allBooks.map((eachBook)=>(
            // <tr key={eachBook.id}>
            //     <td>{eachBook.id}</td>
            //     <td><img src={eachBook.bookImageUrl} width="150px" height="100px" alt="bookImage"></img></td>
            //     <td>{eachBook.bookTitle}</td>
            //     <td>{eachBook.bookAuthor}</td>
            //     <td>{eachBook.bookGenre}</td>
            //     <td>{eachBook.bookCost}</td>
            // </tr>
             <BookItem data={eachBook} key={eachBook.id} onDelete={()=>{this.handleDelete(eachBook)}}></BookItem>
